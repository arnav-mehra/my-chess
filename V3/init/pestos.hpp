/* piece/sq table-based eval */
/* Full Credit: Rofchade & cpwiki. http://www.talkchess.com/forum3/viewtopic.php?f=2&t=68311&start=19 */
...
namespace PeSTOs {
    ...
    int eval(Board& b) {
        ...
        /* evaluate each piece */
        for (int sq = 0; sq < 64; ++sq) {
            Piece pc = b.get_board(sq);
            if (pc != Piece::NA) {
                bool color = (int)pc < 6;
                mg[color] += mg_table[(int)pc][sq];
                eg[color] += eg_table[(int)pc][sq];
                gamePhase += gamephaseInc[(int)pc];
            }
        }

        /* tapered eval */
        int mgScore = mg[1] - mg[0];
        int egScore = eg[1] - eg[0];
